...
> **Deprecated (reference only).** This R document is superseded by
> `RNAseq/quantification.py` (built on `pytximport`), which has been merged
> into `run_rnaseq.sh` as **Phase 4: Quantification**.
> Run `python quantification.py config.yml` (or just run the bash pipeline) to
> produce the same date-stamped xlsx files in `<root>/quantification/`.
