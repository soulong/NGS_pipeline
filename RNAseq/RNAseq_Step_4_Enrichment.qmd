...
params:
  ...
  # pathway of interest for individual analysis
  pathway_of_interest: ["HALLMARK_MYC_TARGETS_V1", "HALLMARK_MYC_TARGETS_V2"]
--- 
...
if(!exists("genesets")) {
  ...
  genesets <- list(
    ...
    Hallmark = filter(msigdb, gs_cat == "H") %>%
      select(gs_id=gs_exact_source, gs_name, gene_symbol) %>%
  ...
  # to list (Hallmark only by default)
  genesets <- list(
    Hallmark = list(
      term2gene = genesets$Hallmark[, c("gs_id", "gene_symbol")],
      term2name = genesets$Hallmark[, c("gs_id", "gs_name")]
    )
  )
}
```
...
```{r}
# mark gene
pathway <- params$pathway_of_interest
