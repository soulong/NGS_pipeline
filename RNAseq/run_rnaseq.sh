...
#> mamba activate ngs
#> bash run_rnaseq.sh config.yml 
## edit xxx/samplesheet.csv (generated by previous command)
#> bash run_rnaseq.sh config.yml
...
parse_yaml_config() {
    ...
    while IFS=': ' read -r key value || [[ -n "$key" ]]; do
        ...
        case "$key" in
            ...
            run_salmon) RUN_SALMON="$value" ;;
            run_star) RUN_STAR="$value" ;;
            run_featurecounts) RUN_FEATURECOUNTS="$value" ;;
            run_quantification) RUN_QUANTIFICATION="$value" ;;
            quantification_dir) QUANTIFICATION_DIR="$value" ;;
...
main() {
    ...
    # Check dependencies
    require_commands fastqc fastp STAR salmon samtools featureCounts multiqc python
    ...
    RUN_SALMON="${RUN_SALMON:-true}"
    RUN_STAR="${RUN_STAR:-false}"
    RUN_FEATURECOUNTS="${RUN_FEATURECOUNTS:-false}"
    RUN_QUANTIFICATION="${RUN_QUANTIFICATION:-true}"
    QUANTIFICATION_DIR="${QUANTIFICATION_DIR:-quantification}"

    # set path
    OUTDIR="$ROOT_DIR/$OUTDIR"
    FASTQ_DIR="$ROOT_DIR/$FASTQ_DIR"
    SAMPLESHEET="$ROOT_DIR/$SAMPLESHEET"
    QUANTIFICATION_DIR="$ROOT_DIR/$QUANTIFICATION_DIR"
    ...
    log_info "  Run Salmon: $RUN_SALMON"
    log_info "  Run STAR: $RUN_STAR"
    log_info "  Run featureCounts: $RUN_FEATURECOUNTS"
    log_info "  Run Quantification: $RUN_QUANTIFICATION"
    log_info "============================================================"
    ...
    if [[ "$RUN_SALMON" == true ]]; then
        log_info ""
        log_info "========== Phase 2: Salmon quantification =========="
        create_dirs "$SALMON_DIR"
        loop_over_csv "$SAMPLESHEET" run_salmon
    else
        log_info ""
        log_info "========== Skip Salmon quantification =========="
    fi
    ...
    if [[ "$RUN_STAR" == true ]]; then
        ...
        create_dirs "$STAR_DIR"
        loop_over_csv "$SAMPLESHEET" run_star

        if [[ "$RUN_FEATURECOUNTS" == true ]]; then
            log_info ""
            log_info "========== Running featureCounts =========="
            create_dirs "$COUNTS_DIR"
            run_featurecounts
         fi
    else
        log_info ""
        log_info "========== Skip STAR alignment =========="
    fi
    
    # Phase 4: Quantification (pytximport)
    if [[ "$RUN_QUANTIFICATION" == true ]]; then
        log_info ""
        log_info "========== Phase 4: Quantification =========="
        create_dirs "$QUANTIFICATION_DIR"
        python "$SCRIPT_DIR/quantification.py" "$config_file"
    else
        log_info ""
        log_info "========== Skip Quantification =========="
    fi
